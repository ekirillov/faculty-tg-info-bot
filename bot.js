...
const replyWithTeachersInlineKeyboard = async (ctx) => {
  ...
  ctx.reply("Список преподавателей", {
    reply_markup: {
      inline_keyboard: generateTeachersInlineKeyboard(teachersGroupedByDepartment)
    }
  })
}
...
const generateTeachersInlineKeyboard = (teachersByDepartment) => {
  ...
  return Object.entries(teachersByDepartment).reduce((res, [department, teachers]) => (
    [
      ...res,
      [{ text: department, callback_data: "do-nothing" }],
